...
Install from github with pip::
    ...
    pip install -e git+https://github.com/myousif9/ciftify_reconall_snakebids#egg=ciftify_reconall_snakebids
...
Do a dry-run first (``-n``) and simply print (``-p``) what would be run::
    ...
    ciftify_reconall_snakebids /path/to/bids/dir /path/to/output/dir participant -np

Run the app, using all cores::
    
    ciftify_reconall_snakebids /path/to/bids/dir /path/to/output/dir participant --cores all
...
To generate a report, run::
    ...
    ciftify_reconall_snakebids /path/to/bids/dir /path/to/output/dir participant --report
...
Here are some instructions to get your python environment set-up on graham to run ciftify_reconall_snakebids:

#. Create a virtualenv and activate it::

      cd $SCRATCH
      module load python/3
      virtualenv venv_ciftify_reconall_snakebids
      source venv_ciftify_reconall_snakebids/bin/activate
...
In an interactive job (for testing)::
    ...
    ciftify_reconall_snakebids bids_dir out_dir participant --participant_label 001 -j 8


Submitting a job (for larger cores, more subjects), still single job, but snakemake will parallelize over the 32 cores::

    regularSubmit -j Fat ciftify_reconall_snakebids bids_dir out_dir participant  -j 32


Scaling up to ~hundred subjects (needs cc-slurm snakemake profile installed), submits 1 16core job per subject::
    
    ciftify_reconall_snakebids bids_dir out_dir participant  --profile cc-slurm
...
Scaling up to even more subjects (uses group-components to bundle multiple subjects in each job), 1 32core job for N subjects (e.g. 10)::
    ...
    ciftify_reconall_snakebids bids_dir out_dir participant  --profile cc-slurm --group-components subj=10
